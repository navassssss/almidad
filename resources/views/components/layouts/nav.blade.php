...
<nav class="container mx-auto flex justify-between w-screen">
    ...
    <div class="relative">
        ...
        <div id="mobileMenuDropdown" style="z-index: 10;"
            ...
            <div class="flex">
                ...
                <a href="{{ route('home') }}"><img class="h-16 m-5 ms-10" src="{{ asset('images/logo.png') }}"
                        alt="logo"></a>
                <hr class="mx-6">
            </div>
    ...
    <div class="hidden md:flex font-bold mt-20 me-10 text-[20px] md:space-x-6 xl:space-x-20">
                    ...
                    <ul>
                        ...
                        @foreach ($moreCategories as $more)
                            <li class="px-4 py-2 text-gray-700 hover:text-[#4484A4] cursor-pointer text-right"
                                data-value="{{ $more->a_name }}">

                                <a href="{{ route('category.show', ['category' => $more->slug]) }}">
                                    {{ $more->a_name }}
                                </a>
                            </li>
                            <hr class="mx-4">
        ...
        @foreach ($mainCategories as $main)
            <a class="hover:text-[#4484A4] text-[#3F3B3B]" href="{{ route('category.show', ['category' => $main->slug]) }}">
                {{ $main->a_name }}
            </a>
    ...
    <div>
        <a href="{{ route('home') }}"><img class="h-20 md:h-24 mt-11 me-5 md:me-0" src="{{ asset('images/logo.png') }}"
                alt="logo"></a>
    </div>
</nav>
<script>
    // Store the last submitted search value globally
    let lastSearchValue = "{{ request('search', '') }}";
