<x-layouts.app title="{{ $category->a_name }}" description="مقالات ومواضيع في قسم {{ $category->a_name }} في مجلة المداد العربية.">
            ...
            <div class="flex flex-wrap justify-center gap-5 mt-10">
                            ...
                            <div
                                ...
                                <div class="relative z-10 text-white">
                                    <h2 class="text-[10px] bg-[#886C8D] inline-block px-2 me-8 font-bold">
                                        {{$article->category->a_name}}</h2>
                                    <h3 class="text-xl mt-2 me-8">{{$article->topic}}</h3>
                                    <p class="mt-1 text-[13px] me-8">{{$article->author->name}}</p>
                                </div>
                            </div>
                        </a>
                    @endforeach
                @else
                    <p dir="rtl" class="text-center  mx-auto font-bold text-3xl  mb-8 pt-5 w-fit ">
                        لا توجد مقالات متعلقة بـ "{{ $category->a_name }}"
                    </p>
                @endif
            </div>
